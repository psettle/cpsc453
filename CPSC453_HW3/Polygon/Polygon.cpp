    ...
    )
{
    ...
    ApplyTranslation(glm::vec3(0.0f));
}
...
void Polygon::OnFrame()
{
    ...
    GLuint model_matrix_uniform = glGetUniformLocation(pShaderM->GetProgramID(), "model_matrix");
    glUniformMatrix4fv(model_matrix_uniform, 1, GL_FALSE, &translationMatrix[0][0]);
...
void Polygon::ApplyTranslation(const glm::vec3& translation)
{
...
Polygon::~Polygon()
{
    if (pFrameDispatcherM != nullptr)
    {
        pFrameDispatcherM->UnregisterFrameListener(this);
    }

    if (buffersToFreeM.size() > 0)
    {
